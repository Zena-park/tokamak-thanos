...
const bridge_1_depositTON_L1_TO_L2 = async (amount: BigNumber) => {
  console.log('\n==== bridge_1_depositTON_L1_TO_L2 1  ====== ')
  ...
  const data = ethers.utils.solidityPack(
    ...
    [l1Wallet.address,  amount.toString(), '0x']
  )
...
const bridge_3_depositETH_To_L1_TO_L2 = async (amount: BigNumber) => {
  ...
  const deposition = await l1BridgeContract
    .connect(l1Wallet)
    .bridgeETHTo(l2TesterWallet.address, amount, 0, '0x', {
      value: amount,
    })
...
const bridge_4_withdrawETH_L2_TO_L1 = async () => {
  ...
  const amount2 = ethers.utils.parseEther('0')  // native ton
  const withdrawal = await l2BridgeContract
    .connect(l2Wallet)
...
const main = async () => {
  ...
  // await bridge_3_depositETH_L1_TO_L2(ethers.utils.parseEther('3'))
