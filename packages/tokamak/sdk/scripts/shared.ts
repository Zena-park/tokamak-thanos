...
import Artifact__ERC20 from '../../contracts-bedrock/forge-artifacts/MockERC20V1.sol/MockERC20V1.json'
...
import Artifact__MockAttack from '../../contracts-bedrock/forge-artifacts/MockAttack.sol/MockAttack.json'
...
export const deployERC20 = async (
  ...
  // const token = await Factory__ERC20.deploy(name, symbol, signer.address)
...
export const deployAttack = async (
  hre: HardhatRuntimeEnvironment,
  signer: Wallet
): Promise<Contract> => {
  const Factory__Attack = new hre.ethers.ContractFactory(
    Artifact__MockAttack.abi,
    Artifact__MockAttack.bytecode.object,
    signer
  )

  console.log('Sending deployment transaction')
  const attack = await Factory__Attack.deploy()
  const receipt = await attack.deployTransaction.wait()
  console.log(`Attack deployed: ${receipt.transactionHash}`)
...
export const differenceLog = async (prevBalance, afterBalance) => {
  ...
  // console.log("prevBalance.OptomismPortalBalance", prevBalance.OptomismPortalBalance)
  // console.log("afterBalance.OptomismPortalBalance", afterBalance.OptomismPortalBalance)
...
export const getBalances = async (
  ...
  console.log('OptomismPortalBalance')
...
export const getPortalDepositedAmount = async (portal) => {
  // const balance = await await token.balanceOf(portal.address)
  return {
